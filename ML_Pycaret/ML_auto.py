...
#Model
    ...
    if data[target].dtypes in ['object','bool'] or len(pd.unique(data[target])) <= 10:
        ...
        option = st.selectbox("Choose Bulid Model Method:", ['',"Auto", "Specific"])
        if option=='Auto':
            ...
            with open('best_model.pkl', 'rb') as model:
                st.download_button('Download the model', model, 'best_model.pkl')
        else:
            setup(data, target=target)
            setup = pull()
            st.info("Data SetUp")
            ...
            choose = st.selectbox("Choose Model:", models)
